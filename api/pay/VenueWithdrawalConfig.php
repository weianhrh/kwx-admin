...
$db->query("CREATE TABLE IF NOT EXISTS venue_withdrawal_configs (
    ...
    withdraw_ratio DECIMAL(5,2) NOT NULL DEFAULT 20.00 COMMENT '加盟商实际提现比例(%)',
...
if ($action === 'save') {
    ...
    if ($ratio < 0 || $ratio > 100) out(1002, '提现比例必须在0%到100%之间');
    if ($fee < 0 || $fee > 100) out(1002, '手续费必须在0%到100%之间');
    if ($ratio + $fee > 100) out(1002, '提现比例与手续费合计不能超过100%');
